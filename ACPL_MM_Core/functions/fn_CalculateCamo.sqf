...
while {(_unit getvariable ["ACPL_MM_Core_Camo", false]) && (alive _unit) && (_custom || (isPlayer _unit) || (_unit in allPlayers))} do {
	private _timenow = daytime;
	
	private _foglevel = linearconversion [0,1,fog,0,0.9,true];
	
	if (uniform _unit != _unit) then {
		_camo = 0.55;
	...
	private _newCamCoefScaled = linearconversion [0,1,_newCamCoef,0,1.6,true];
	private _newaudibleCoefScaled = linearconversion [0,1,_newCamCoefA,0.05,1.6,true];
