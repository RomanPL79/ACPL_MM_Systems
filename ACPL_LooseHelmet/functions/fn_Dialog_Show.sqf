/*
	ACPL_LooseHelmet_fnc_dialog_Create
*/
...
if (
	count _data == 0 ||
	!createDialog "ACPL_LooseHelmet_GUI_Module"
) exitwith {
	false
};

// Gets display info and makes sure it's created
private _display = uiNamespace getVariable ["ACPL_LooseHelmet_GUI_Module_Display", -1];
...
private _elements = [];
{
	...
	switch (_type) do {
		...
		case "SLIDER": {
			...
			_element setvariable ["ACPL_LooseHelmet_GUI_Slider", _textField];
			_element setvariable ["ACPL_LooseHelmet_GUI_Slider_range", _range];
			_textField setvariable ["ACPL_LooseHelmet_GUI_Slider", _element];

			// Adds eventHandlers for slider
			_element ctrlAddEventHandler ["SliderPosChanged", {
				params ["_element", "_value"];

				private _textField = _element getvariable ["ACPL_LooseHelmet_GUI_Slider", -1];
			...
			_textField ctrlAddEventHandler ["KeyUp", {
				...
				private _element = _textField getvariable ["ACPL_LooseHelmet_GUI_Slider", -1];
				private _range = _element getvariable ["ACPL_LooseHelmet_GUI_Slider_range", [0,0]];
...
_confirm ctrlSetText localize "STR_ACPL_LooseHelmet_module_button_confirm";
...
_confirm ctrlAddEventHandler ["ButtonClick", {
	...
	private _completeFunction = _display getVariable ["ACPL_LooseHelmet_display_complete", {}];
	private _data = [_display] call ACPL_LooseHelmet_fnc_dialog_getData;
...
_cancel ctrlSetText localize "STR_ACPL_LooseHelmet_module_button_cancel";
...
_display setVariable ["ACPL_LooseHelmet_display_complete", _exec];
_display setVariable ["ACPL_LooseHelmet_display_elements", _elements];